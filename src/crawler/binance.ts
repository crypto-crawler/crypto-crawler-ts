...
export default async function crawl(
  ...
  const dataHandler = async (data: WebSocket.Data) => {
    ...
    switch (channelType) {
      case 'BBO': {
        ...
        const market = marketMap.get(rawBookTickerMsg.s)!;
        ...
        if (market.type === 'Futures' || (market.type === 'Swap' && market.quote === 'USD')) {
          msg.bidQuantity =
            (parseFloat(rawBookTickerMsg.B) * (market.base === 'BTC' ? 100 : 10)) / msg.bidPrice;
          msg.askQuantity =
            (parseFloat(rawBookTickerMsg.A) * (market.base === 'BTC' ? 100 : 10)) / msg.askPrice;
        }
      ...
      case 'OrderBook': {
        ...
        const market = marketMap.get(rawOrderbookMsg.s)!;
        ...
        const parseOrder = (arr: Array<string>): OrderItem => {
          ...
          if (market.type === 'Futures' || (market.type === 'Swap' && market.quote === 'USD')) {
            orderItem.cost = parseFloat(arr[1]) * (market.base === 'BTC' ? 100 : 10);
            orderItem.price = parseFloat(arr[0]);
            orderItem.quantity = orderItem.cost / orderItem.price;
          } else {
            orderItem.price = parseFloat(arr[0]);
            orderItem.quantity = parseFloat(arr[1]);
      ...
      case 'Kline': {
        ...
        const market = marketMap.get(rawKlineMsg.s)!;
        ...
        if (market.type === 'Futures' || (market.type === 'Swap' && market.quote === 'USD')) {
          klineMsg.volume = parseFloat(rawKlineMsg.k.q);
          klineMsg.quoteVolume = parseFloat(rawKlineMsg.k.v) * (market.base === 'BTC' ? 100 : 10);
        }
      ...
      case 'Ticker': {
        ...
        const market = marketMap.get(rawTickerMsg.s)!;
        ...
        if (market.type === 'Futures' || (market.type === 'Swap' && market.quote === 'USD')) {
          tickerMsg.volume = parseFloat(rawTickerMsg.q);
          tickerMsg.quoteVolume = parseFloat(rawTickerMsg.v) * (market.base === 'BTC' ? 100 : 10);
        }
      ...
      case 'Trade': {
        if (rawMsg.stream.split('@')[1] === 'trade') {
          ...
          const market = marketMap.get(rawTradeMsg.s)!;
          ...
          if (market.type === 'Futures' || (market.type === 'Swap' && market.quote === 'USD')) {
            msg.quantity =
              (parseFloat(rawTradeMsg.q) * (market.base === 'BTC' ? 100 : 10)) / msg.price;
          }
  ...
  if (marketType !== 'Swap') {
    connect(websocketUrl, dataHandler, undefined);
  } else {
    if (markets.filter((m) => m.quote === 'USD').length > 0) {
      const websocketUrlCoin = `${WEBSOCKET_ENDPOINTS.Futures}/stream?streams=${getChannels(
        marketType,
        channelTypes,
        pairs.filter((p) => p.endsWith('_USD')),
        markets.filter((m) => m.quote === 'USD'),
        getChannel,
      ).join('/')}`;

      connect(websocketUrlCoin, dataHandler, undefined);
    }

    if (markets.filter((m) => m.quote === 'USDT').length > 0) {
      const websocketUrlUSDT = `${WEBSOCKET_ENDPOINTS.Swap}/stream?streams=${getChannels(
        marketType,
        channelTypes,
        pairs.filter((p) => p.endsWith('_USDT')),
        markets.filter((m) => m.quote === 'USDT'),
        getChannel,
      ).join('/')}`;

      connect(websocketUrlUSDT, dataHandler, undefined);
    }
  }
}
